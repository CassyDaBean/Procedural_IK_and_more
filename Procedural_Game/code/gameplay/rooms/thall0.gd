...
export (String) var InternType:String = "thall"
...
func Adjust():
	...
	if !AdjArray.empty():
		for i in AdjArray.size():
			...
			if (AdjArray[2].InternType == "lhall" || AdjArray[2].InternType == "4way" || AdjArray[2].InternType == "thall"):
				if self.rotation_degrees.y == 0:
					AdjArray[2].global_transform.origin = Side2.global_transform.origin + Vector3(-15,0,0)
				if self.rotation_degrees.y == -180:
					AdjArray[2].global_transform.origin = Side2.global_transform.origin + Vector3(15,0,0)
				if self.rotation_degrees.y == 90:
					AdjArray[2].global_transform.origin = Side2.global_transform.origin + Vector3(0,0,15)
				if self.rotation_degrees.y == -90:
					AdjArray[2].global_transform.origin = Side2.global_transform.origin + Vector3(0,0,-15)
			if AdjArray[2].InternType == "hall":
				AdjArray[2].global_transform.origin = Side2.global_transform.origin
